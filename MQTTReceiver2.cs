...
public class mqttReceiver2 : M2MqttUnityClient
{

    [Header("MQTT topics")]
    [Tooltip("Set the topic to subscribe. !!!ATTENTION!!! multi-level wildcard # subscribes to all topics")]
    public string topicSubscribe = "cdanh2012/feeds/mqtt-subscribe1"; // topic to subscribe. !!! The multi-level wildcard # is used to subscribe to all the topics. Attention i if #, subscribe to all topics. Attention if MQTT is on data plan
                                                                  
    [Tooltip("Set the topic to publish (optional)")]
    public string topicPublish = "cdanh2012/feeds/mqtt-subscribe1"; // topic to publish
    public string messagePublish = "HuhuHahHa"; // message to publish
    ...
    protected override void SubscribeTopics()
    {
        ...
        // client.Subscribe(new string[] { topicSubscribe2 }, new byte[] { MqttMsgBase.QOS_LEVEL_EXACTLY_ONCE });
    }

    protected override void UnsubscribeTopics()
    {
        client.Unsubscribe(new string[] { topicSubscribe });
        //   client.Unsubscribe(new string[] { topicSubscribe2 });
    }
